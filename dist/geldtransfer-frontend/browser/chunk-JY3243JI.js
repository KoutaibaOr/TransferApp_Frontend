...
var CsvService = class _CsvService {
   ...
   */
  export(data, headers, filename) {
    if (!data.length)
      return;
    const BOM = "\uFEFF";
    const headerRow = headers.map((h) => this.escape(h.label)).join(";");
    const rows = data.map((row) => headers.map((h) => this.escape(String(row[h.key] ?? ""))).join(";"));
  ...
  static {
    this.\u0275fac = function CsvService_Factory(t) {
      return new (t || _CsvService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _CsvService, factory: _CsvService.\u0275fac, providedIn: "root" });
  }
};
...
//# sourceMappingURL=chunk-JY3243JI.js.map
